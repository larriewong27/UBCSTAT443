# Multiple regression equations and diagnostics: Burnaby condominium data
# Initial plots, regression with different subsets, residual plots

b<-read.table("bcondo-161116.txt",header=T,skip=2) # data frame
print(names(b))
...
b$ffarea<-b$ffarea/100; b$mfee=b$mfee/10
b$sqfl<-sqrt(b$floor)
...
# see  Section 3.4, for explanations/definitions
...
# residual plots, compare Figure 3.3 of the coursepack
...
abline(h=2*sigma7); abline(h=-2*sigma7); abline(h=0)
